...
__global__ void gemm_warp_tiling_kernel (
    ...
    float threadResults[(WNITER * TNWARPS) * (WMITER * TMWARPS)];
    float regM[WMITER * TMWARPS];
    float regN[WNITER * TNWARPS];
    for (int i = 0; i < (WNITER * TNWARPS) * (WMITER * TMWARPS); ++i) threadResults[i] = 0.0f;
    for (int i = 0; i < WMITER * TMWARPS; ++i) regM[i] = 0.0f;
    for (int i = 0; i < WNITER * TNWARPS; ++i) regN[i] = 0.0f;
