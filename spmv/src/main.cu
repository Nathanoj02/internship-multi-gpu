...
int main() {
    // test_load_balancing();
    // File
    std::string matrix_file = "data/vas_stokes_4M.mtx";
    ...
    float* arr = generate_array(rows, 0.0f, 10.0f);
    float* result = new float[rows];

    // Generate load balancing plan
    size_t *row_mapping = balance_load(nnz, STREAM_NUM, row_offset, rows);
    ...
    for (int i = -WARMUP_RUNS; i < TIMED_RUNS; ++i) {
        start = std::clock();
        spmv(result, arr, row_offset, cols, values, rows, nnz);
        end = std::clock();
