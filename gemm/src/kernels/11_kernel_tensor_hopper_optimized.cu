...
#include <mma.h>
#include "kernels/11_kernel_tensor_hopper_optimized.cuh"
#include "gemm.cuh"
...
// =====================================================================
// Optimized Hopper GEMM — persistent warpgroup pipeline
// TMA bulk async copy + WGMMA + cluster multicast
// =====================================================================
...
__host__ static inline CUtensorMap create_tensor_map(half* gmem_ptr, int global_height, int global_width) {
    ...
    static_assert(BlockMinorSize >= 64, "BlockMinorSize must be >= 64");
    assert(global_width % 64 == 0);

    // Specifying dimensions from fastest (contiguous) to slowest
    ...
    // Similarly: BK and then BM/BN (for A/B)
...
// =====================================================================
// Global state for TMA map and schedule caching
// =====================================================================
...
// =====================================================================
// WGMMA m64nNk16 instruction wrappers (f32 accumulation, f16 inputs)
// =====================================================================
...
__device__ __forceinline__ void wgmma(float d[WGMMA_N/16][8], half* sA, half* sB) {
    static_assert(WGMMA_N == 128 || WGMMA_N == 192 || WGMMA_N == 256, "WGMMA_N must be 128, 192, or 256");
...
__device__ static inline void load_async(half* dst, void const* src_tma_map, uint64_t* bar, int global_col_idx, int global_row_idx) {
    ...
    // 3D tiling: first dim is 64 columns, second is row, third is col/64.
    // Required for tiles wider than 64 elements.
    asm volatile(
...
__device__ void arrive_cluster(uint64_t* bar, uint32_t cta_id, uint32_t count = 1) {
    ...
    // Call arrive on barrier stored in SM number `cta_id` via mapa.shared
    asm volatile(
        "{\n"
        ".reg .b32 remAddr32;\n"
...
__device__ static inline void load_async_multicast(half* dst, void const* src_tma_map, uint64_t* bar, int global_col_idx, int global_row_idx, uint16_t cluster_mask) {
    uint64_t tma_ptr  = reinterpret_cast<uint64_t>(src_tma_map);
    uint32_t mbar_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(bar));
    uint32_t dst_ptr  = static_cast<uint32_t>(__cvta_generic_to_shared(dst));
...
// =====================================================================
// Persistent tile scheduler using Hilbert curve ordering
// =====================================================================
...
struct SMem {
    ...
    alignas(128) half C[BN * (BM + (BM / 64) * 8)]; // 8-element padding per consumer warpgroup
    alignas(8)   uint64_t full[QSIZE], empty[QSIZE];
    int space[SPACE_LEN];
};

// =====================================================================
// Main kernel: persistent producer-consumer pipeline
// =====================================================================

template<int BM, int BN, int BK, int NUM_THREADS, int QSIZE, int NUM_SM, int CLUSTER_M, int CLUSTER_N>
__global__ __launch_bounds__(NUM_THREADS) void __cluster_dims__(CLUSTER_M * CLUSTER_N, 1, 1)
matmulKernel12(int M, int N, int K,
    ...
    int* dspace)
{
    ...
    // Load this cluster's schedule into shared memory
    ...
    Schedule<2, NUM_SM / CLUSTERS, BM * CLUSTER_M, BN * CLUSTER_N, 16 / CLUSTER_M, 8 / CLUSTER_N>
    ...
    if (wg_idx == 0) {
        ...
        if (tid == 0) {
            ...
            while (schedule.next(num_block_m, num_block_n)) {
                ...
                for (int block_k_iter = 0; block_k_iter < num_blocks_k; ++block_k_iter, ++qidx) {
                    ...
                    expect_bytes(&full[qidx], (BK * BN + BK * BM) * sizeof(half));
        ...
        // Signal all pipeline slots as empty
        for (int qidx = 0; qidx < QSIZE; ++qidx)
            if (tid < CLUSTERS) arrive_cluster(&empty[qidx], tid);

        // stmatrix and wgmma layouts match for f16 (see PTX guide sections 9.7.14.5.16 and Figure 148)
        int lane = tid % 32, warp = tid / 32;
        half*    block_sC  = sC + wg_idx * B_WG_M_PADDED * BN;
        uint32_t tid_offset = warp * 16 + (lane % 8) * B_WG_M_PADDED;
        tid_offset += (lane / 16) * B_WG_M_PADDED * 8 + (lane & 8);
        uint32_t base_addr = static_cast<uint32_t>(__cvta_generic_to_shared(block_sC)) + tid_offset * sizeof(half);
        ...
        while (schedule.next(num_block_m, num_block_n)) {
            ...
            // First K-block: initialize accumulators (ScaleD=0)
            {
                if (qidx == QSIZE) { qidx = 0; p ^= 1; }
                wait(&full[qidx], p);
                warpgroup_arrive();
            ...
            // Remaining K-blocks: accumulate (ScaleD=1)
            ...
            // Store accumulators to shared memory using stmatrix, then TMA store to global
            half     d_half[8];
            int*     data_ptr = (int*)d_half;
            for (int m_it = 0; m_it < B_WG_M / WGMMA_M; ++m_it) {
                for (int w = 0; w < WGMMA_N; w += 16) {
                    uint32_t addr = base_addr + (w * B_WG_M_PADDED + m_it * WGMMA_M) * sizeof(half);
                    for (int k = 0; k < 8; k++) d_half[k] = (half)(d[m_it][w / 16][k]);
                    asm volatile(
...
// =====================================================================
// Hilbert curve tile scheduler (host)
// =====================================================================
...
void createHilbert(int M, int N, int CORES, int* space) {
    ...
    for (int i = 0; i < FCORES; ++i) {
        if ((int)pos.back().size() >= (int)pos[0].size() - 1) break;
...
void runKernel12(int M, int N, int K, half* A, half* B, half* C) {
...
__global__ void half_to_float_kernel(const half* src, float* dst, int n) {
...
// =====================================================================
// Public wrapper — matches project interface (half in, float out)
// Device allocations are cached per matrix size to keep TMA maps valid.
// =====================================================================
...
void gemm_tensor_hopper_optimized(
    ...
    // Cache device allocations so TMA maps remain valid across benchmark iterations
    static half*  d_A       = nullptr;
    static half*  d_B       = nullptr;
    static half*  d_C       = nullptr;
    static float* d_C_float = nullptr;
    static size_t prev_rows_a = 0, prev_cols_a = 0, prev_cols_b = 0;

    if (rows_a != prev_rows_a || cols_a != prev_cols_a || cols_b != prev_cols_b) {
        cudaFree(d_A); cudaFree(d_B); cudaFree(d_C); cudaFree(d_C_float);

        CUDA_CHECK( cudaMalloc(&d_A,       rows_a * cols_a * sizeof(half)) );
        CUDA_CHECK( cudaMalloc(&d_B,       rows_b * cols_b * sizeof(half)) );
        CUDA_CHECK( cudaMalloc(&d_C,       rows_a * cols_b * sizeof(half)) );
        CUDA_CHECK( cudaMalloc(&d_C_float, rows_a * cols_b * sizeof(float)) );

        prev_rows_a = rows_a; prev_cols_a = cols_a; prev_cols_b = cols_b;
    }

    CUDA_CHECK( cudaMemcpy(d_A, A, rows_a * cols_a * sizeof(half), cudaMemcpyHostToDevice) );
    CUDA_CHECK( cudaMemcpy(d_B, B, rows_b * cols_b * sizeof(half), cudaMemcpyHostToDevice) );

    runKernel12(rows_a, cols_b, cols_a, d_A, d_B, d_C);

    CUDA_CHECK( cudaGetLastError() );
    CUDA_CHECK( cudaDeviceSynchronize() );

    // Convert half → float and copy back to host
    constexpr int threads = 256;
    half_to_float_kernel<<<CEIL_DIV(rows_a * cols_b, threads), threads>>>(d_C, d_C_float, rows_a * cols_b);
    CUDA_CHECK( cudaDeviceSynchronize() );

    CUDA_CHECK( cudaMemcpy(result, d_C_float, rows_a * cols_b * sizeof(float), cudaMemcpyDeviceToHost) );
}
...
void gemm_tensor_hopper_optimized(
    ...
    fprintf(stderr, "Error: Hopper optimized kernel requires sm_90 (Hopper architecture).\n"
